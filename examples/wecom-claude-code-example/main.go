...
import (
	...
	claudecode "github.com/IMBotPlatform/LLMClaudeCode/pkg"
...
const (
	...
	glmTimeoutMS      = "3000000"
	glmDisableTraffic = "1"
	glmOpusModel      = "GLM-4.7"
	glmSonnetModel    = "GLM-4.7"
	glmHaikuModel     = "GLM-4.5-Air"
)
...
func loadEnvConfig() envConfig {
	...
	if cfg.anthropicAuthToken == "" {
		missing = append(missing, "ANTHROPIC_AUTH_TOKEN")
	}
	if len(missing) > 0 {
...
func main() {
	...
	// 2) 初始化 LLM（Claude Code + GLM Anthropic 兼容接口）。
	llm, err := newGLMLLM(cfg.anthropicAuthToken)
	if err != nil {
...
// newGLMLLM 创建基于 Claude Code CLI 的 GLM 模型实例。
// 参数：authToken 为 GLM/Anthropic 兼容接口密钥。
// 返回：llms.Model 与错误。
func newGLMLLM(authToken string) (llms.Model, error) {
	if strings.TrimSpace(authToken) == "" {
		return nil, fmt.Errorf("ANTHROPIC_AUTH_TOKEN is required")
	}

	// 参考 llm_glm_test.go 固定参数配置。
	env := map[string]string{
		"ANTHROPIC_AUTH_TOKEN":                     authToken,
		"ANTHROPIC_BASE_URL":                       glmBaseURL,
		"API_TIMEOUT_MS":                           glmTimeoutMS,
		"CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC": glmDisableTraffic,
		"ANTHROPIC_DEFAULT_OPUS_MODEL":             glmOpusModel,
		"ANTHROPIC_DEFAULT_SONNET_MODEL":           glmSonnetModel,
		"ANTHROPIC_DEFAULT_HAIKU_MODEL":            glmHaikuModel,
	}

	// 与 llm_glm_test.go 一致的初始化参数。
	return claudecode.New(
		claudecode.WithModel(""),
		claudecode.WithSystemPrompt(""),
		claudecode.WithCLIPath(""),
